...
registerFormatType( 'piotrpress/dynamic-format', {
    title: __( 'Dynamic format', 'dynamic-format' ),
    tagName: 'span',
    className: 'dynamic-format',
    edit: function ( { contentRef, isActive, onChange, value } ) {
        const anchorRef = useAnchor( { ref: contentRef, value } );
        const [ showPopover, setShowPopover ] = useState( false );

        const getCallback = () => {
            const { activeFormats } = value;
            const formats = activeFormats.filter( format => 'piotrpress/dynamic-format' === format['type'] );

            if ( formats.length > 0 ) {
                const format = formats[0];
                const { attributes, unregisteredAttributes } = format;

                if ( unregisteredAttributes && unregisteredAttributes.hasOwnProperty( 'data-callback' ) ) {
                    return unregisteredAttributes['data-callback'];
                } else if ( attributes && attributes.hasOwnProperty( 'data-callback' ) ) {
                    return attributes['data-callback'];
                }
            }
        ...
        return (
            ...
            { showPopover && (
                <URLPopover
                    anchorRef={ anchorRef }
                    className="dynamic-format-popover"
                    onClose={ () => setShowPopover( false ) }
                >
                    <SelectControl
                        label={ __( 'Select callback', 'dynamic-format' ) }
                        value={ getCallback() }
                        options={ dynamicFormatCallbacks }
                        onChange={ ( callback ) => {
                            setShowPopover( false );
                            if ( callback ) {
                                onChange(
                                    applyFormat( value, {
                                            type: 'piotrpress/dynamic-format',
                                            attributes: { 'data-callback': callback },
                                        }
                                    ) );
                            } else {
                                onChange( toggleFormat( value, { type: 'piotrpress/dynamic-format' } ) ); // Remove Format.
                            }
                        } }
